...
export class StudentComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && StudentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && StudentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IStudent[] | null): IStudent[] {
    ...
    if (this.links().prev) {
      const studentsNew = this.students ?? [];
      if (data) {
        for (const d of data) {
          if (studentsNew.some(op => op.id === d.id)) {
            studentsNew.push(d);
          }
        }
      }
